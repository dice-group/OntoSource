  ...
  const handleAddAxiom = () => {
    ...
    switch (selectedAxiomType) {
      ...
      case 'object-property':
        ...
        addObjectPropertyMutation.mutate({ namespace, property_name: propertyName })
        break
      ...
      case 'data-property':
        ...
        addDataPropertyMutation.mutate({ namespace, property_name: propertyName })
        break
      ...
      case 'object-property-assertion':
        if (!subjectIri || !objectIri || !opPropertyName || !opPropertyNamespace) {
          alert('Please provide all required fields')
          return
        }
      ...
      case 'data-property-assertion':
        if (!dpSubjectIri || !dpPropertyName || !dpPropertyNamespace || !literalValue) {
          alert('Please provide all required fields')
          return
        }
      ...
      case 'subclass-of':
        if (!subclassName || !subclassNamespace || !superclassName || !superclassNamespace) {
          alert('Please provide all required fields')
          return
        }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto p-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Ontology Editor</h1>
          <p className="text-gray-600">Create and modify ontologies with ease</p>
        </div>
        
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg border shadow-sm p-6 mb-6">
              <h2 className="text-lg font-medium text-gray-900 mb-4">Select or Create Ontology</h2>
              ...
              {isCreatingNew && (
                <div className="mb-4 p-4 bg-gray-50 rounded-md">
                  ...
                  />
                  <p className="text-xs text-gray-600 mb-2">IRI should end with #, /, or :</p>
                  <input
                    type="text"
                  ...
                  >
                    {createOntologyMutation.isPending ? 'Creating...' : 'Create'}
                  </button>
                </div>
              )}

              <div className="space-y-2">
                <h3 className="text-sm font-medium text-gray-700">Existing Ontologies</h3>
                {data?.summary && data.summary.length > 0 ? (
                  data.summary.map((ontology: Ontology) => (
                    ...
                    >
                      <p className="font-medium text-sm text-gray-900">{ontology.filename}</p>
                      <p className="text-xs text-gray-600 truncate">{ontology.id}</p>
                    </div>
                  ))
                ) : (
                  <p className="text-sm text-gray-500">No ontologies available</p>
                )}
              </div>
            </div>
            ...
            {selectedOntology && (
              <div className="bg-white rounded-lg border shadow-sm p-6">
                <h3 className="text-lg font-medium text-gray-900 mb-2">Current Namespace</h3>
                <p className="text-sm text-gray-900 break-all font-mono bg-gray-50 p-2 rounded">
                  {namespace || 'No namespace set'}
                </p>
                
                <div className="mt-4 pt-4 border-t">
                  <h4 className="text-sm font-medium text-gray-700 mb-2">Save Ontology</h4>
                  <input
                    type="text"
                  ...
                  >
                    {saveOntologyMutation.isPending ? 'Saving...' : 'Save Ontology'}
                  </button>
                </div>
              </div>
            )}
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-lg border shadow-sm p-6">
              <h2 className="text-lg font-medium text-gray-900 mb-4">Add Axioms</h2>
              
              {!selectedOntologyId ? (
                <p className="text-gray-500">Please select or create an ontology to start editing</p>
              ) : (
                <>
                  <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
                    <p className="text-sm text-blue-800">
                      <strong>Note:</strong> Namespaces should end with #, /, or : (they will be auto-corrected with # if missing)
                    </p>
                  </div>
                  <div className="mb-6">
                    ...
                    >
                      ...
                      <option value="object-property">Add Object Property</option>
                      <option value="data-property">Add Data Property</option>
                      <option value="individual">Add Individual</option>
                      <option value="class-assertion">Add Class Assertion</option>
                      <option value="object-property-assertion">Add Object Property Assertion</option>
                      <option value="data-property-assertion">Add Data Property Assertion</option>
                      ...
                      <>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            className="w-full px-3 py-2 border rounded-md text-gray-900 placeholder-gray-500"
                            placeholder={selectedAxiomType === 'object-property' ? 'hasParent' : 'hasAge'}
                          />
                        </div>
                      </>
                    )}
                      ...
                      <>
                        ...
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              const selectedClass = entitiesData?.classes.find(c => c.name === e.target.value)
                              setAssertionClassName(e.target.value)
                              if (selectedClass) {
                                const iri = selectedClass.iri
                                const namespace = iri.substring(0, iri.lastIndexOf(selectedClass.name))
                      ...
                      <>
                        ...
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              const selectedProp = entitiesData?.object_properties.find(p => p.name === e.target.value)
                              setOpPropertyName(e.target.value)
                              if (selectedProp) {
                                const iri = selectedProp.iri
                                const namespace = iri.substring(0, iri.lastIndexOf(selectedProp.name))
                      ...
                      <>
                        ...
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              const selectedProp = entitiesData?.data_properties.find(p => p.name === e.target.value)
