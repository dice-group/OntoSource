...
export function getNodeType(uri: string): NodeType {
  ...
  if (lower.includes('owl#class') || lower.includes('rdfs#class')) return 'class'
  if (lower.includes('#class') && !lower.includes('property')) return 'class'
  if (lower.includes('/class') && !lower.includes('property')) return 'class'
  
  // Check for OWL/RDF/RDFS Properties
  if (lower.includes('owl#objectproperty') || lower.includes('owl#datatypeproperty')) return 'property'
  if (lower.includes('owl#annotationproperty') || lower.includes('rdf#property')) return 'property'
  if (lower.includes('rdfs#property')) return 'property'
  if (lower.includes('#property') || lower.includes('/property')) return 'property'
  
  // Check for common OWL/RDFS resources that are properties
  if (lower.includes('rdfs#subclassof') || lower.includes('rdfs#subpropertyof')) return 'property'
  if (lower.includes('rdfs#domain') || lower.includes('rdfs#range')) return 'property'
  if (lower.includes('rdfs#label') || lower.includes('rdfs#comment')) return 'property'
  if (lower.includes('owl#equivalentclass') || lower.includes('owl#sameas')) return 'property'
...
export function isSchemaRelation(predicate: string): boolean {
  ...
  return lower.includes('w3.org/1999/02/22-rdf-syntax-ns#') ||
...
export function generateColorFromString(str: string, saturation = 70, lightness = 50): string {
  let hash = 0
...
export function getLinkColor(predicate: string, propertyColorMap?: Map<string, string>): string {
