...
export default function KnowledgeGraph({ ontologyId, width }: KnowledgeGraphProps) {
 ...
 const [layoutDirection, setLayoutDirection] = useState<'TB' | 'LR'>('TB')
  ...
  const { data, isPending, error } = useQuery({...createGraphDataQueryOptions(ontologyId || ''), enabled: !!ontologyId})
  ...
  const { schemaRelations, ontologyRelations } = useMemo(() => {
    ...
    const schemaPredicates: Array<{ uri: string; label: string; color: string }> = []
    const ontologyPredicates: Array<{ uri: string; label: string; color: string }> = []
  ...
  const { nodes, edges, visibleTripleCount, triples, total, filteredTotal } = useMemo(() => {
    ...
    allTriples.forEach((t: Triple) => {
      ...
      if (pLower.includes('rdf-syntax-ns#type') || pLower.endsWith('#type') || pLower.endsWith('/type')) {
        if (typeof o === 'string') {
          const oLower = o.toLowerCase()
          
          // Check for property types
          if (oLower.includes('objectproperty') || oLower.includes('datatypeproperty')) {
            typeMap.set(s, 'property')
          } else if (oLower.includes('annotationproperty') || oLower.includes('functionalproperty')) {
            typeMap.set(s, 'property')
          } else if (oLower.includes('rdf#property') || oLower.includes('rdfs#property')) {
            typeMap.set(s, 'property')
          } 
          // Check for class types
          else if (oLower.includes('owl#class') || oLower.includes('rdfs#class')) {
            typeMap.set(s, 'class')
    ...
    allTriples.forEach((t: Triple) => {
      ...
      if (hiddenNodeTypes.has(subjectType) || hiddenNodeTypes.has(objectType)) includeTriple = false
    ...
    const triples = maxTriplesToDisplay > 0 && maxTriplesToDisplay < filteredTriples.length
    ...
    triples.forEach((t: Triple, i: number) => {
      ...
      if (!nodeMap.has(s)) {
        ...
        nodeMap.set(s, { id: s, label: shortenUri(s), type: nt, boxType: bt, color: getNodeColor(nt) })
      }
      ...
      if (typeof o === 'string') {
        ...
        objectLabel = o.value.length > 30 ? o.value.slice(0, 30) + '…' : o.value
        objectType = 'literal'
        objectBoxType = 'literal'
      }
  
      if (!nodeMap.has(objectId)) {
        nodeMap.set(objectId, { id: objectId, label: objectLabel, type: objectType, boxType: objectBoxType, color: getNodeColor(objectType) })
      }
    ...
    const filteredEdges = edgesAcc.filter((e: any) => {
      ...
      if (e.data?.predicate && hiddenRelations.has(e.data.predicate)) return false
      return true
    })
    ...
    triples.forEach((t: Triple, i: number) => {
      ...
      if (filteredNodeIds.has(s) && filteredNodeIds.has(objectId) && !hiddenRelations.has(p)) {
        visibleCount++
      }
    })
    ...
    const layouted = getLayoutedElements(rfNodes, filteredEdges, layoutDirection)
    return { 
      ...layouted, 
  ...
  }, [data, layoutDirection, showTBox, showABox, showLiterals, hiddenNodeTypes, hiddenRelations, maxTriplesToDisplay])
    ...
    return (
      <div className="min-h-full bg-white rounded-lg border shadow-sm p-8 flex items-center justify-center ">
        <div className="text-center">
          ...
          <p className="text-gray-500 text-sm mt-1">Please select an ontology from the list to visualize its knowledge graph</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-full bg-white rounded-lg border shadow-sm p-8 flex items-center justify-center ">
        <div className="flex flex-col items-center">
          ...
          <p className="text-gray-500 text-sm mt-1">Preparing visualization data</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="bg-white rounded-lg border shadow-sm flex flex-col h-screen">
      <div className="p-4 border-b space-y-3 flex-shrink-0">
        <div>
          ...
          <p className="text-sm text-gray-500">
            ...
            {total > 0 && ` (displaying ${triples.length} of ${filteredTotal} filtered triples, ${total} total)`}
          </p>
        </div>
        
        {/* Triple display limit control */}
        <div className="flex items-center gap-4">
          <label htmlFor="triple-limit" className="text-sm font-medium text-gray-700 whitespace-nowrap">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              const newValue = Math.min(Math.max(100, Number(e.target.value)), maxValue)
      ...
      <div className="flex-1 min-h-0" style={{ width: width ?? '100%' }}>
        {!hasData ? (
          <div className="h-full flex items-center justify-center bg-gray-50">
            <div className="text-gray-600">No triples available in this ontology</div>
          </div>
        ) : !hasVisibleTriples ? (
          <div className="h-full flex items-center justify-center bg-gray-50">
            <div className="text-center">
              <div className="text-gray-600 mb-2">No triples visible with current filters</div>
              <div className="text-sm text-gray-500">Try adjusting TBox/ABox/Literal visibility or unhiding node types/relations in the legend below</div>
            </div>
          </div>
        ) : (
          <ReactFlow
      ...
      <div className="p-4 border-t bg-gray-50 flex-shrink-0">
        ...
        <div className="mb-3">
          <h5 className="text-xs font-medium text-gray-700 mb-1">Knowledge Types</h5>
          <div className="flex flex-wrap gap-4 text-xs text-gray-700">
            <div className="flex items-center space-x-1">
              <div className="w-12 h-6 bg-blue-500 rounded border-2 border-black flex items-center justify-center text-white text-[10px] font-semibold">TB</div>
              <span>TBox (Schema: Classes & Properties)</span>
            </div>
            <div className="flex items-center space-x-1">
              <div className="w-12 h-6 bg-orange-500 rounded border-2 border-dashed border-black flex items-center justify-center text-white text-[10px]">AB</div>
              <span>ABox (Instances: Individuals)</span>
            </div>
            <div className="flex items-center space-x-1">
              <div className="w-12 h-6 bg-red-500 rounded border border-dotted border-black flex items-center justify-center text-white text-[10px] italic">Lit</div>
        ...
        <div className="mb-3">
          <h5 className="text-xs font-medium text-gray-700 mb-1">Node Types (click to hide/show)</h5>
          <div className="flex flex-wrap gap-4 text-xs text-gray-700">
            <LegendDot 
              color="#3b82f6" 
              label="Classes" 
        ...
        {schemaRelations.length > 0 && (
          <div className="mb-3">
            <h5 className="text-xs font-medium text-gray-700 mb-1">
              ...
              <span className="ml-1 text-gray-500 font-normal">(RDF/RDFS/OWL - click to hide/show)</span>
            </h5>
        ...
        {ontologyRelations.length > 0 && (
          <div>
            <h5 className="text-xs font-medium text-gray-700 mb-1">
              ...
              <span className="ml-1 text-gray-500 font-normal">(Domain-specific - click to hide/show)</span>
            </h5>
            <div className="flex flex-wrap gap-3 text-xs text-gray-700 max-h-32 overflow-y-auto">
...
function LegendDot({ color, label, onClick, isHidden }: { color: string; label: string; onClick?: () => void; isHidden?: boolean }) {
  return (
    <div 
      ...
      onClick={onClick}
      title={onClick ? (isHidden ? `Click to show ${label}` : `Click to hide ${label}`) : undefined}
    >
